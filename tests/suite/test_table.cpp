...
TEST_SUITE(table, "table", {
  DESCRIBE("skip", {
    IT("should add an offset", {
      ...
      assert_equal(sm.to_sql().c_str(), "SELECT FROM \"users\" OFFSET 2");
    })
  })
  ...
  DESCRIBE("having", {
    IT("adds a having caluse", {
      ...
      assert_equal(sm.to_sql().c_str(), "SELECT FROM \"users\" HAVING \"users\".\"id\" = 10");
    })
  })
  ...
  DESCRIBE("backwards compat", {
    IT("join noops on null", {
      ...
      assert_equal(sm.to_sql().c_str(), "SELECT FROM \"users\"");
    })
    ...
    IT("takes a second argument for join type", {
      ...
      assert_equal(mgr.to_sql().c_str(), "SELECT FROM \"users\" LEFT OUTER JOIN \"users\" \"users_2\" ON \"users\".\"id\" = \"users_2\".\"id\"");
    })
  })
  ...
  DESCRIBE("group", {
    IT("should create a group", {
      ...
      assert_equal(mgr.to_sql().c_str(), "SELECT FROM \"users\" GROUP BY \"users\".\"id\"");
    })
  })
  ...
  DESCRIBE("order", {
    IT("should take an order", {
      ...
      assert_equal(mgr.to_sql().c_str(), "SELECT FROM \"users\" ORDER BY foo");
    })
  })
  ...
  DESCRIBE("take", {
    IT("should add a limit", {
      ...
      assert_equal(mgr.to_sql().c_str(), "SELECT * FROM \"users\" LIMIT 1");
    })
  })
  ...
  DESCRIBE("project", {
    IT("can project", {
      ...
      assert_equal(mgr.to_sql().c_str(), "SELECT * FROM \"users\"");
    })
  ...
  DESCRIBE("where", {
    IT("returns a tree manager", {
      ...
      assert_equal(mgr.to_sql().c_str(), "SELECT \"users\".\"id\" FROM \"users\" WHERE \"users\".\"id\" = 1");
    })
  })
