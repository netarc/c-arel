...
namespace c_arel {
  ...
  Table::Table(const char *name, Connection *connection) : name(name), connection(connection) {
  }

  SelectManager Table::from(Table &table) {
    return SelectManager(this->connection ? this->connection : &_base_connection, table);
  }
...
//  }
  ...
  DeleteManager Table::delete_from(void) {
    return DeleteManager(this->connection, *this);
  }
